...
http.createServer(function(req,res){
	...
	if (req.url === '/' || req.url ==='/index.html' && req.method === 'get') {//index.html
		...
		fs.readFile(path.join(__dirname, 'data', 'data.json'), 'utf8', function(err, data){
			...
			var list = JSON.parse(data || '[]');
			// 2 获取用户post提交的数据--- 数据量可能非常大，会分多次进行提交
			// 想要获取用户提交所有数据，需要监听request事件的data事件（浏览器是一部分一部分提交的）
			// request.end事件触发时，浏览器把所有数据都提交到了服务器了

			// 监听request的data 和 end事件：
			var array = [];// 保存用户每次提交过来的数据
			req.on('data',function(chunk){// chunk是浏览器本次提交过来的数据，类型是buffer
				array.push(chunk);
			});
			// 监听request的end事件-触发则数据提交完毕
			req.on('end',function(){// 汇总array中的buffer对象
				var postBody = Buffer.concat(array);
				// 把获取到的buffer对象转成一个字符串
